...
public:
  ...
  struct Iterator{
    ...
    constexpr Iterator operator--(int) {Iterator copy{*this}; --*this; return copy;}
    constexpr Iterator& operator+=(size_t i) {curr += static_cast<int>(i); return *this;}
    constexpr Iterator& operator-=(size_t i) {curr -= static_cast<int>(i); return *this;}
    constexpr Iterator operator+(size_t i) const {Iterator copy{*this}; copy += static_cast<int>(i); return copy;}
    constexpr Iterator operator-(size_t i) const {Iterator copy{*this}; copy -= static_cast<int>(i); return copy;}
  ...
  constexpr Iterator begin() const {
    return Iterator{.curr=_min};
  }

  constexpr Iterator end() const {
    return Iterator{.curr=_max+1};
  }
...
int main() {
  ...
  for(int i : std::views::all(testDie) | std::views::transform([](int i){return i * 2 - 1;})) {
    std::print("{}\n", i);
  }
}
