...
namespace probx {
  ...
  namespace detail {
    constexpr void composite_impl(dice::MappedRoll::Map & map, OutcomeStack & outcomeStack, Compositor auto && func) {
    ...
    constexpr void composite_impl(dice::MappedRoll::Map & map, OutcomeStack & outcomeStack, Compositor auto && func, Rollable auto && roll, Rollable auto&& ...rolls) {
      for(auto outcome : roll) {
	outcomeStack.push_back(std::make_pair(outcome, roll(outcome)));
	composite_impl(map, outcomeStack, std::forward<decltype(func)>(func), std::forward<decltype(rolls)>(rolls)...);
	outcomeStack.pop_back();
      }
    }
  }
      
  constexpr dice::MappedRoll composite(Compositor auto && func, Rollable auto && ...rolls) {
    OutcomeStack outcomeStack;
    dice::MappedRoll::Map map;
    detail::composite_impl(map, outcomeStack, std::forward<decltype(func)>(func), std::forward<decltype(rolls)>(rolls)...);
  ...
  constexpr dice::MappedRoll composite(Compositor auto && func, Rolls && rolls) {
    ...
    if(size == 0) {
      ...
      return composite(std::forward<decltype(func)>(func), *std::ranges::begin(rolls));
    } else if(size == 2) {
      ...
      auto mappedRoll1 = composite(std::forward<decltype(func)>(func), *std::ranges::begin(chunks));
      auto mappedRoll2 = composite(std::forward<decltype(func)>(func), *std::ranges::rbegin(chunks));
      return composite(std::forward<decltype(func)>(func), mappedRoll1, mappedRoll2);
    }
  }
  ...
  constexpr dice::MappedRoll ternaryComposite(
					      TernaryCompositor auto && func,
