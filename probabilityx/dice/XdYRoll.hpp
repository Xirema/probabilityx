...
namespace probx::dice {
  ...
  public:
    ...
    constexpr XdYRoll(Integer numOfDice, Integer sizeOfDice): numOfDice(numOfDice), sizeOfDice(sizeOfDice) {}
    ...
    constexpr Iterator end() const{return Iterator{.curr=numOfDice * sizeOfDice + 1};}
	...
	BigInt value = choose(numOfDice, k) * choose(o.result - sizeOfDice * k - 1, o.result - sizeOfDice * k - numOfDice);
	if(k % 2 == 0) {
	  ret += value;
	} else {
	  ret -= value;
