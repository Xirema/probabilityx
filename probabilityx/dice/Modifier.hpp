...
namespace probx::dice {
  ...
  public:
    ...
    struct Iterator{
      ...
      constexpr Iterator& operator++() {curr.result++; return *this;}
      constexpr Iterator& operator--() {curr.result--; return *this;}
      constexpr Iterator operator++(int) {Iterator copy{*this}; ++*this; return copy;}
      constexpr Iterator operator--(int) {Iterator copy{*this}; --*this; return copy;}
      constexpr Iterator& operator+=(size_t i) {curr.result += static_cast<Integer>(i); return *this;}
      constexpr Iterator& operator-=(size_t i) {curr.result -= static_cast<Integer>(i); return *this;}
      ...
      constexpr Outcome operator[](Integer i) const {return Outcome{curr.result + i, curr.extra};};
      constexpr Integer operator-(Iterator const& o) const {return curr.result - o.curr.result;}
      constexpr auto operator<=>(Iterator const& o) const = default;
    };

    constexpr Iterator begin() const {return Iterator{.curr=mod};}
    constexpr Iterator end() const{return Iterator{.curr=Outcome{mod.result+1, mod.extra}};}
