...
namespace probx {
  ...
  struct Outcome {
    ...
    constexpr Outcome(Integer result, Integer extra): result(result), extra(extra) {}
...
namespace std {
  template<> struct hash<probx::Outcome> {
  ...
  template<> struct formatter<probx::BigInt> : formatter<string> {
  ...
  template<> struct formatter<probx::Rational> {
  ...
  template<> struct formatter<probx::Outcome> {
