...
struct std::hash<probx::RollVariant> {
  constexpr size_t operator()(probx::RollVariant const& v) const {
    return std::visit(
        [](probx::Rollable auto&& roll) { return probx::hash(roll); }, v);
  }
};

template <>
struct std::equal_to<probx::RollVariant> {
  constexpr bool operator()(probx::RollVariant const& v1,
                            probx::RollVariant const& v2) {
    return std::visit(
        [&v2](probx::Rollable auto&& r1) {
          return std::visit(
              [&r1](probx::Rollable auto&& r2) {
...
class RollFactory {
 ...
 public:
  ...
  constexpr RollVariant const& getXdYRoll(
      ...
      std::optional<Outcome> threshold = {}) {
    Rules rules{.numOfDice = numOfDice,
    ...
    if (numOfDice <= 0) {
      ...
      return xdyRolls[rules] = composite(
                 compositors::d20::rerollOnce(*threshold), base, base);
    } else {
      for (Integer power = 0;
           power < static_cast<Integer>(std::log2(numOfDice)); power++) {
        Rules newRules = rules;
        newRules.numOfDice = 2 << power;
        if (xdyRolls.find(newRules) != xdyRolls.end()) {
          continue;
        }
        xdyRolls[newRules] = std::visit(
            [](Rollable auto&& roll) {
              return composite(compositors::adder, roll, roll);
            },
      ...
      while (remainingDice > 0) {
        Rules newRules = rules;
        Integer largestPowerOfTwo =
            1 << static_cast<Integer>(std::log2(remainingDice));
        ...
        auto const& newRet = xdyRolls[finalRules] = std::visit(
            [ret](Rollable auto&& roll1) {
              return std::visit(
                  [&roll1](Rollable auto&& roll2) {
