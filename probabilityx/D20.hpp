...
namespace probx::d20 {
  ...
  constexpr dice::MappedRoll getAttackRoll(AttackRules rules) {
    std::ranges::sort(rules.damageDice, [](dice::RegularDie const& a, dice::RegularDie const& b) {
      return mean(a) > mean(b);
    });
    std::vector<Outcome> d20Outcomes;
    for(Integer i = 1; i <= 20; i++) {
      d20Outcomes.push_back(Outcome{i, i <= rules.missRange ? -1 : i >= (21-rules.critRange) ? 1 : 0});
    }
    dice::CustomDie d20{d20Outcomes};
    dice::MappedRoll attackRoll = composite(compositors::adder, d20, dice::Modifier(rules.attackMod));
	...
	for(auto const& die : rules.damageDice) {
	  dice::MappedRoll rerolledDie = composite(compositors::d20::rerollOnce(*rules.damageRerollThreshold), die, die);
	...
	if(rules.damageRerollThreshold) {
	  auto rerolledDie = composite(compositors::d20::rerollOnce(*rules.damageRerollThreshold), largestDamageDie, largestDamageDie);
    ...
    if(damageRoll.size() > 0) {
      damageRoll = composite(compositors::adder, damageRoll, dice::Modifier{rules.damageMod});
    } else {
      damageRoll = composite(compositors::adder, dice::Modifier{rules.damageMod});
    }
    if(critRoll.size() > 0) {
      critRoll = composite(compositors::adder, critRoll, dice::Modifier{rules.damageMod}, dice::Modifier{rules.extraCritFlat});
    } else {
      critRoll = composite(compositors::adder, dice::Modifier{rules.damageMod}, dice::Modifier{rules.extraCritFlat});
    }

    return ternaryComposite(detail::AttackRollCompositor{rules.targetAC}, attackRoll, damageRoll, critRoll);
  }
}
